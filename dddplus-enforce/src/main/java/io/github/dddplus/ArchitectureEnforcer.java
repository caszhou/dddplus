...
public class ArchitectureEnforcer {
    ...
    public static final ArchRule serviceRule() {
        return classes().that()
            ...
            .should()
            .implement(IDomainService.class)
            .andShould()
    ...
    public static final ArchRule abilityRule() {
        return classes().that()
            .haveNameMatching(".*Ability")
            .should()
            .beAssignableTo(BaseDomainAbility.class)
            .andShould()
            .beAnnotatedWith(DomainAbility.class)
            .as("ability必须继承BaseDomainAbility，并且加@DomainAbility");
    }

    public static final ArchRule specificationRule() {
        return classes().that()
            .implement(ISpecification.class)
            .should()
            .beAnnotatedWith(Specification.class)
            .as("ISpecification rule");
    }

    public static final ArchRule partnerRule() {
        return classes().that().haveNameMatching(".*Partner").should().beAnnotatedWith(Partner.class).as("Partner使用规范");
    }

    public static final ArchRule noActivityClassAllowed() {
        return noClasses().should().haveNameMatching(".*Activity").as("Activity必须定义成Step");
    }
    ...
    public static final ArchRule domainStepRule() {
        return classes().that()
            .implement(IDomainStep.class)
            .and()
            .doNotHaveModifier(JavaModifier.ABSTRACT) // 排除Activity
            .should()
            .haveSimpleNameEndingWith("Step")
            .andShould()
    ...
    public static final ArchRule patternRule() {
        return classes().that()
            ...
            .should()
            .beAssignableTo(IIdentityResolver.class)
            .andShould()
    ...
    public static final ArchRule extensionRule() {
        return classes().that()
            .areAssignableTo(IDomainExtension.class)
            .and()
            .areNotInterfaces()
            .and()
            .haveNameNotMatching(".Default*") // 默认的扩展点实现可以不使用 @Extension
            .should()
            .haveNameMatching(".*Ext")
            .andShould()
            .beAnnotatedWith(Extension.class)
            .as("扩展点实现的规范");
    }
    ...
    public static final ArchRule creatorRule() {
        return classes().that()
            .haveNameMatching(".*Creator")
            .should()
            .implement(IDomainModelCreator.class)
            .as("Creator rule");
    }
    ...
    public static final ArchRule optionalInterfaceNameStartsWithI() {
        return classes().that()
            .areInterfaces()
            .and()
            .areNotAnnotatedWith(Target.class) // 把注解本身排除掉
            .and()
            .haveNameNotMatching(".*Dao")
            .and()
            .haveNameNotMatching(".*Manager")
            .and()
            .haveNameNotMatching(".*Api")
            .and()
            .haveNameNotMatching(".*Group") // 排除JSR303 group interface
            .should()
    ...
    public static final ArchRule controllers_should_only_use_their_own_slice() {
        return slices().matching("..controller.(*)..")
