...
public class InternalIndexer {
    ...
    static final Map<String, Map<String, StepDef>> domainStepDefMap = new HashMap<>(); // {activityCode, {stepCode,
                                                                                       // def}}
    ...
    public static <T extends BaseDomainAbility> T findDomainAbility(@NotNull Class<? extends T> clazz) {
        ...
        if (domainAbilityDef == null) {
            ...
            log.error("{} forgot to apply @DomainAbility, ArchitectureEnforcer turned off? indexed:{}",
                clazz.getCanonicalName(), domainAbilityDefMap.keySet());
            return null;
        }
     ...
     * @param clazz
     */
    public static Class<? extends IDomainExtension>
        getDomainAbilityExtDeclaration(@NotNull Class<? extends BaseDomainAbility> clazz) {
        DomainAbilityDef domainAbilityDef = domainAbilityDefMap.get(clazz);
        if (domainAbilityDef == null) {
            // 研发忘记使用注解DomainAbility了，线上bug
            // 但如果没有关闭架构守护神ArchitectureEnforcer，则不可能出现该bug
     ...
     */
    @NotNull
    public static List<ExtensionDef> findEffectiveExtensions(@NotNull Class<? extends IDomainExtension> extClazz,
        @NotNull IDomainModel model, boolean firstStop) {
    ...
    static void index(DomainAbilityDef domainAbilityDef) {
        ...
        if (domainAbilityDefMap.containsKey(domainAbilityDef.getDomainAbilityClass())) {
            throw BootstrapException.ofMessage("duplicated domain ability: ",
                domainAbilityDef.getDomainAbilityBean().toString());
        }
    ...
    static void index(PolicyDef policyDef) {
        if (policyDefMap.containsKey(policyDef.getExtClazz())) {
            // 一个扩展点定义只能有一个策略实例
            throw BootstrapException.ofMessage("1 Policy decides only 1 Extension:", policyDef.policyName(), ", ext:",
                policyDef.getExtClazz().getCanonicalName());
        }
    ...
    static void prepare(ExtensionDef extensionDef) {
        if (partnerDefPrepared == null) {
            ...
            throw BootstrapException.ofMessage("Partner must reside in Plugin Jar with its extensions!"); // TODO
                                                                                                          // Partner的定义没有出现在Plugin
                                                                                                          // Jar的场景可能也需要支持
        }
