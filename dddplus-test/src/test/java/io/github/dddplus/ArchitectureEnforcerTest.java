...
public class ArchitectureEnforcerTest {
    ...
    public void setUp() {
        ...
        classes = classes.that(not(belongToAnyOf(
            // 这些类是故意写错的，要排除掉
            EggExt.class, IllegalDomainAbility.class,
    ...
    public void optionalInterfaceNameStartsWithI() {
        try {
            ...
            assertTrue(expected.getMessage()
                .contains("Architecture Violation [Priority: MEDIUM] - Rule '接口名称必须以I开头' was violated"));
            assertTrue(expected.getMessage().contains(" violated (5 times)"));
            assertTrue(expected.getMessage()
                .contains(
                    "simple name of io.github.dddplus.runtime.registry.mock.ability.AbilityTag does not start with 'I'"));
            assertTrue(expected.getMessage()
                .contains(
                    "simple name of io.github.dddplus.runtime.registry.mock.pattern.Patterns does not start with 'I'"));
            assertTrue(expected.getMessage()
                .contains("simple name of io.github.dddplus.runtime.registry.mock.step.Steps does not start with 'I'"));
            assertTrue(expected.getMessage()
                .contains(
                    "simple name of io.github.dddplus.runtime.registry.mock.step.Steps$Cancel does not start with 'I'"));
            assertTrue(expected.getMessage()
                .contains(
                    "simple name of io.github.dddplus.runtime.registry.mock.step.Steps$Submit does not start with 'I'"));
        }
    }
}
